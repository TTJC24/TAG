...
batch intake plus one disabled-by-default Outlook draft capability:
...
- a provider-neutral execution seam with `deterministic_internal` as the
  default and a Mail `drafts.create` provider gated by approved task, enabled
...
write implementation creates an unsent Outlook draft and ships inert; sending
...
3. Keep every future connector capability separately reviewed; Mail remains
...
6. Do not add Mail sending or another external write during this handoff.
...
The implementation stops at the passing Outlook draft external-write slice.
