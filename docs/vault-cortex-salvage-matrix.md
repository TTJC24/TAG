...
| Evaluation findings | `gbrain-eval/PLAY_A_REPORT.md`, `gbrain-eval/PLAY_B_REPORT.md`, `gbrain-eval/UPSTREAM_BUG_ANALYSIS.md`, `cortex/samson-cortex/retrieval/evals/*` | Company Brain has citation evals plus `bun run source-isolation:check`, which covers source-scoped search, missing-provenance refusal, and empty requested source lanes. | Keep | Preserve remaining Cortex gold-query/eval harness ideas. Do not replace the current cited answer path without stronger recall/citation evidence. | Need sensitivity-reviewed fixture conversion for any broader gold-query cases. |
